  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-8 sm:px-8 sm:py-12">
      ...
      <div className="card-soft space-y-5 p-6">
        ...
        <Field label="Restricciones">
          <div className="flex flex-wrap gap-2">
            {RESTS.map((p) => (
              <button
                key={p}
                onClick={() => toggle(rests, setRests, p)}
        ...
        <button onClick={save} className="w-full rounded-full bg-primary py-3 text-sm font-medium text-primary-foreground">
          Guardar cambios
        </button>
      </div>
      ...
      {/* Suppress unused variable warnings for constants defined for future use */}
      {false && GOALS && LEVELS && TIMES}
    </div>
  );
}
